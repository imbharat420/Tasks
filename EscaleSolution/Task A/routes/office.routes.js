...
router.post('/office', async (req, res) => {
  ...
  if (!name || !addressLine1 || !country) return res.status(400).json({ message: 'Please fill all the fields' });
  try {
    const office = await Office.findOne({ name });

    if (office) return res.status(400).json({ message: 'Office already exists by this name' });
